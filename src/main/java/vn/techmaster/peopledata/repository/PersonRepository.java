...
/*  //2.1 Nhóm những người trong cùng thành phố lại
  ...
  public Map.Entry<String, Long> topJobInPeopleGroup(List<Person> peopleGroup) {
    Optional<Map.Entry<String, Long>> result = peopleGroup
            ...
            .entrySet()
            .stream().max(Map.Entry.comparingByValue()); //Tìm lớn nhất
  ...
  public List<Map.Entry<String,Double>> fiveCitiesHasTopAverageSalary() {
    ...
    return getEntries(groupPeopleByCity);
  }
  ...
  public List<Map.Entry<String,Double>> fiveCitiesHasTopSalaryForDeveloper() {
    ...
    return getEntries(groupDevelopersByCity);
...
/*
  private List<Map.Entry<String, Double>> getEntries(Map<String, List<Person>> groupDevelopersByCity) {
    ...
    /*
      ...
      public Double maleFemaleRatio() {
        long maleCount = people
                .stream()
                .filter(p -> p.getGender().equals("Male")).count();
    ...
    */
    public Map<String, List<Person>> groupDeveloperByCity() {
        ...
        Map<String, List<Person>> map = people
                .stream()
                .collect(Collectors.groupingBy(Person::getJob));
        return map.get("developer").stream().collect(Collectors.groupingBy(Person::getCity));

    }

    public List<Map.Entry<String, Double>> youngestDeveloperCity() {

        Map<String, List<Person>> youngestDeveloperCity = people
                .stream()
                .filter(p -> p.getJob().equals("developer"))
                .collect(Collectors.groupingBy(Person::getCity));

        return getTop1(youngestDeveloperCity);

    }

    private List<Map.Entry<String, Double>> getTop1(Map<String, List<Person>> groupDevelopersByCity) {
        Map<String, Double> cityAverageAgeForDeveloper = groupDevelopersByCity.entrySet()
                .stream()
                .collect(Collectors.toMap(Map.Entry::getKey,
                        entry -> entry.getValue()
                                .stream()
        ...
        return cityAverageAgeForDeveloper.entrySet()
                .stream()
  ...
  public Double maleFemaleRatio() {
    ...
    public Map<String, Double> maleFemaleRatioByCity() {
        Map<String, List<Person>> groupPersonByCity = people
                .stream()
                .collect(Collectors.groupingBy(Person::getCity));
